...
import { 
  ...
  gerarSorteioMataMata,     
  adicionarFaseAvancada,
  avancarFaseMataMata // <--- IMPORTANTE: Nova função importada
} from '../../../actions'
...
export default function GerenciarLiga() {
  ...
  const [viewMode, setViewMode] = useState<'lista' | 'chave'>('lista')
  ...
  // --- NOVA FUNÇÃO PARA AVANÇAR FASE ---
  async function handleAvancarFase(fase: number) {
    if(!confirm(`Deseja gerar os confrontos da fase ${fase + 1} com base nos vencedores desta fase?`)) return
    
    const res = await avancarFaseMataMata(campeonatoId, fase)
  ...
  // --- HELPER: CARD DE JOGO ATUALIZADO (GRID LAYOUT) ---
  const MatchCard = ({ jogo }: { jogo: any }) => (
    <div key={jogo.id} className={`relative rounded-xl border shadow-md transition-all duration-200 overflow-hidden ${editingId === jogo.id ? 'bg-blue-900/30 border-blue-500 z-50 ring-2 ring-blue-500' : 'bg-[#1a1a1a] border-gray-800 hover:border-gray-600'}`}>
        ...
        <div className="flex items-center justify-between p-3 gap-2">
            ...
            <div className="flex items-center gap-1 shrink-0">
                <input type="number" className="w-10 h-8 bg-black border border-blue-500 text-white text-center font-bold rounded focus:outline-none" value={tempCasa} onChange={e => setTempCasa(e.target.value)} />
                <span className="text-gray-500 font-bold text-xs">x</span>
                <input type="number" className="w-10 h-8 bg-black border border-blue-500 text-white text-center font-bold rounded focus:outline-none" value={tempVisitante} onChange={e => setTempVisitante(e.target.value)} />
            </div>
            <div className="text-left w-1/3 truncate font-bold text-gray-400 text-xs">{jogo.visitante?.nome}</div>
            
            <div className="flex flex-col gap-1 ml-1">
                <button onClick={() => salvarEdicao(jogo.id)} className="bg-green-600 w-6 h-6 rounded hover:bg-green-500 flex items-center justify-center text-white text-[10px]">✓</button>
                <button onClick={() => setEditingId(null)} className="bg-red-600 w-6 h-6 rounded hover:bg-red-500 flex items-center justify-center text-white text-[10px]">✕</button>
            </div>
        </div>
        ) : (
        // MODO VISUALIZAÇÃO (USANDO GRID PARA ALINHAMENTO PERFEITO)
        <div className="grid grid-cols-[1fr_auto_1fr] items-center p-3 gap-3 cursor-pointer relative" onClick={() => iniciarEdicao(jogo)} title="Clique para editar">
            
            {/* Mandante (Alinhado à direita) */}
            <div className="flex items-center justify-end gap-2 overflow-hidden">
                <span className={`font-bold text-xs truncate ${jogo.status === 'finalizado' && jogo.placar_casa < jogo.placar_visitante ? 'text-gray-500' : 'text-gray-200'}`}>
                    {jogo.casa?.nome}
                </span>
                <img src={jogo.casa?.escudo} className={`w-8 h-8 object-contain ${jogo.status === 'finalizado' && jogo.placar_casa < jogo.placar_visitante ? 'grayscale opacity-50' : ''}`} />
            </div>

            {/* Placar Central */}
            <div className={`px-2 py-1 rounded border min-w-[50px] text-center shrink-0 ${jogo.status === 'finalizado' ? 'bg-black border-green-900/30' : 'bg-black border-gray-800'}`}>
                <div className="text-sm font-black font-mono text-white whitespace-nowrap">
                    {jogo.placar_casa ?? '-'} <span className="text-gray-600 mx-px">:</span> {jogo.placar_visitante ?? '-'}
                </div>
            </div>

            {/* Visitante (Alinhado à esquerda) */}
            <div className="flex items-center justify-start gap-2 overflow-hidden">
                <img src={jogo.visitante?.escudo} className={`w-8 h-8 object-contain ${jogo.status === 'finalizado' && jogo.placar_visitante < jogo.placar_casa ? 'grayscale opacity-50' : ''}`} />
                <span className={`font-bold text-xs truncate ${jogo.status === 'finalizado' && jogo.placar_visitante < jogo.placar_casa ? 'text-gray-500' : 'text-gray-200'}`}>
                    {jogo.visitante?.nome}
                </span>
            </div>
            ...
            {/* Botão Excluir (Hover) */}
            <button 
                onClick={async (e) => { e.stopPropagation(); if(confirm("Apagar partida?")) { await supabase.from('partidas').delete().eq('id', jogo.id); carregarDados(); } }}
                className="absolute top-1 right-1 text-red-500 opacity-0 hover:opacity-100 transition text-[10px] hover:bg-red-900/20 p-1 rounded"
            >
                🗑️
            </button>
        </div>
  ...
  const rodadasPresentes = [...new Set(partidas.map(p => p.rodada))].sort((a, b) => a - b)
    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto text-white pb-20 font-sans min-h-screen bg-black">
        ...
        <div className="space-y-8 animate-fadeIn">
          ...
          <div className="bg-gray-900 p-6 rounded-xl border border-gray-800">
            <div className="flex justify-between items-center mb-6">
               ...
               <button onClick={handleSorteio} disabled={poteA.length === 0} className="bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-6 rounded-lg transition disabled:opacity-50 text-sm uppercase tracking-wider">
                 Realizar Sorteio
               </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* COLUNA 1 */}
              <div className="bg-black/50 p-4 rounded-lg border border-gray-700 h-[400px] flex flex-col">
              ...
              {/* COLUNA 2 */}
              <div className="bg-blue-900/10 p-4 rounded-lg border border-blue-500/30 h-[400px] flex flex-col">
                <h3 className="text-blue-400 font-bold mb-3 uppercase text-xs flex justify-between">Pote A (Mandantes) <span>{poteA.length}</span></h3>
                <div className="space-y-2 overflow-y-auto pr-2 custom-scrollbar flex-1">
                  {poteA.map(t => (
                    <button key={t.id} onClick={() => moverParaPote(t, 'B')} className="w-full flex items-center gap-2 p-2 bg-blue-900/40 hover:bg-blue-800/50 rounded text-left border border-blue-800 transition"><img src={t.escudo} className="w-6 h-6" /><span className="text-xs font-bold truncate text-white">{t.nome}</span><span className="ml-auto text-[10px] opacity-50">➜ B</span></button>
                  ))}
                </div>
              </div>
              {/* COLUNA 3 */}
              <div className="bg-red-900/10 p-4 rounded-lg border border-red-500/30 h-[400px] flex flex-col">
                <h3 className="text-red-400 font-bold mb-3 uppercase text-xs flex justify-between">Pote B (Visitantes) <span>{poteB.length}</span></h3>
                <div className="space-y-2 overflow-y-auto pr-2 custom-scrollbar flex-1">
                  {poteB.map(t => (
                    <button key={t.id} onClick={() => moverParaPote(t, 'Livre')} className="w-full flex items-center gap-2 p-2 bg-red-900/40 hover:bg-red-800/50 rounded text-left border border-red-800 transition"><img src={t.escudo} className="w-6 h-6" /><span className="text-xs font-bold truncate text-white">{t.nome}</span><span className="ml-auto text-[10px] opacity-50">✕</span></button>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-900 p-6 rounded-xl border border-gray-800">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-end">
              <div className="flex-1 w-full">
                ...
                <select className="w-full bg-black border border-gray-700 rounded-lg p-3 outline-none" value={faseAvancadaTime} onChange={(e) => setFaseAvancadaTime(e.target.value)}>
                  <option value="">Selecione...</option>
                  {todosTimes.map(t => <option key={t.id} value={t.id}>{t.nome}</option>)}
                </select>
              </div>
              <div className="w-full md:w-32">
                <label className="block text-xs font-bold text-gray-500 mb-2 uppercase">Fase</label>
                <input type="number" className="w-full bg-black border border-gray-700 rounded-lg p-3 outline-none text-center font-bold" value={faseAvancadaRodada} onChange={(e) => setFaseAvancadaRodada(e.target.value)} />
              </div>
              <button onClick={handleAdicionarFaseAvancada} className="w-full md:w-auto bg-yellow-600 hover:bg-yellow-500 text-black font-bold px-6 py-3 rounded-lg transition">Adicionar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {tabAtiva === 'jogos' && (
        <div className="animate-fadeIn">
              ...
              <div className="flex gap-2 relative">
                <div className="relative flex-1">
                  <button onClick={() => setMenuAberto(!menuAberto)} className="w-full bg-black border border-gray-700 text-left px-4 py-3 rounded-lg flex items-center justify-between hover:border-cartola-gold transition">
                    {timeSelecionadoObjeto ? <div className="flex items-center gap-3"><img src={timeSelecionadoObjeto.escudo} className="w-6 h-6" /><span className="font-bold text-white">{timeSelecionadoObjeto.nome}</span></div> : <span className="text-gray-500">Selecione...</span>}
                  ...
                  {menuAberto && (
                    <div className="absolute top-full left-0 w-full mt-2 bg-gray-800 border border-gray-600 rounded-lg shadow-2xl z-50 max-h-60 overflow-y-auto">
                      {timesDisponiveis.map(t => (
                        <button key={t.id} onClick={() => { setSelecionadoId(String(t.id)); setMenuAberto(false); }} className="w-full flex items-center gap-3 p-3 hover:bg-gray-700 border-b border-gray-700/50 text-left"><img src={t.escudo} className="w-8 h-8" /><div className="font-bold text-white text-sm">{t.nome}</div></button>
                      ))}
                    </div>
                  )}
                </div>
              ...
              <div className="h-40 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {timesLiga.map((item) => (
                    <div key={item.id} className="flex justify-between items-center p-2 bg-black/40 rounded border border-gray-800 hover:border-gray-600">
                      <div className="flex items-center gap-3"><img src={item.times.escudo} className="w-8 h-8" /><span className="font-bold text-sm text-gray-300">{item.times.nome}</span></div>
          ...
          {/* MATA-MATA: BOTÃO DE VISUALIZAÇÃO */}
          {liga?.tipo === 'mata_mata' && (
            <div className="flex justify-end mb-4">
                <div className="bg-gray-900 p-1 rounded-lg flex gap-1">
                    <button onClick={() => setViewMode('lista')} className={`px-4 py-1.5 rounded-md text-xs font-bold transition ${viewMode === 'lista' ? 'bg-gray-700 text-white' : 'text-gray-500'}`}>Lista</button>
                    <button onClick={() => setViewMode('chave')} className={`px-4 py-1.5 rounded-md text-xs font-bold transition ${viewMode === 'chave' ? 'bg-blue-600 text-white' : 'text-gray-500'}`}>👁️ Chaveamento</button>
                </div>
            </div>
          )}
          ...
          <div className="bg-gray-900 p-8 rounded-2xl border border-gray-800 shadow-2xl">
            ...
            {/* VISUALIZAÇÃO CHAVEAMENTO */}
            {liga?.tipo === 'mata_mata' && viewMode === 'chave' ? (
                <div className="flex overflow-x-auto gap-12 pb-4 min-h-[400px] px-2 items-start">
                    {rodadasPresentes.map((rod) => (
                        <div key={rod} className="min-w-[320px] flex flex-col shrink-0">
                            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-800">
                                <h3 className="font-bold text-gray-400 uppercase text-xs tracking-widest">{rod}ª Fase</h3>
                                <button onClick={() => handleAvancarFase(rod)} className="text-[10px] bg-blue-900/30 text-blue-300 hover:bg-blue-900/50 px-2 py-1 rounded border border-blue-900 transition flex items-center gap-1">
                                  <span>⚙️</span> Gerar Próxima
                                </button>
                            </div>
                            <div className="flex flex-col gap-6">
                                {partidas.filter(p => p.rodada === rod).map(jogo => (
                                    <MatchCard key={jogo.id} jogo={jogo} />
                                ))}
                            </div>
                        </div>
                    ))}
                    {rodadasPresentes.length === 0 && <p className="text-gray-500 text-center w-full my-auto">Nenhum confronto criado.</p>}
                </div>
            ) : (
            ...
            <>
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 border-b border-gray-800 pb-8 gap-6">
                    <div className="flex-1 w-full">
                        <label className="block text-[10px] mb-2 text-blue-400 font-black uppercase tracking-widest">
                        {liga?.tipo === 'mata_mata' ? 'Fase do Mata-Mata' : 'Rodada do Camp.'}
                        </label>
                        <div className="flex items-center bg-black rounded-lg border border-gray-700 overflow-hidden">
                        <button onClick={() => rodada > '1' && setRodada(String(Number(rodada)-1))} className="w-12 h-12 bg-gray-800 hover:bg-gray-700 text-white font-bold text-xl border-r border-gray-700">‹</button>
                        <select className="flex-1 bg-black text-white font-bold text-lg h-12 text-center outline-none appearance-none cursor-pointer hover:bg-gray-900 transition" value={rodada} onChange={e => setRodada(e.target.value)}>
                            ...
                            <option key={r} value={r}>{liga?.tipo === 'mata_mata' ? `${r}ª Fase` : `Rodada ${r}`}</option>
                            ))}
                        </select>
                        <button onClick={() => setRodada(String(Number(rodada)+1))} className="w-12 h-12 bg-gray-800 hover:bg-gray-700 text-white font-bold text-xl border-l border-gray-700">›</button>
                        </div>
                    </div>
                ...
                <div className="flex flex-col md:flex-row gap-3 items-center bg-black/30 p-4 rounded-xl border border-gray-800 mb-8 relative z-30">
                    <div className="relative w-full">
                        ...
                        <div className="absolute top-full left-0 w-full mt-1 bg-gray-800 border border-gray-600 rounded-lg shadow-2xl z-50 max-h-60 overflow-y-auto">
                            {timesLiga.map(item => <button key={item.time_id} onClick={() => { setTimeCasa(String(item.time_id)); setMenuMandanteAberto(false); }} className="w-full flex items-center gap-3 p-3 hover:bg-gray-700 border-b border-gray-700/50 text-left"><img src={item.times.escudo} className="w-6 h-6" /><span className="text-sm font-bold">{item.times.nome}</span></button>)}
                        </div>
                        )}
                    </div>
                    ...
                    <div className="relative w-full">
                        ...
                        {menuVisitanteAberto && (
                        <div className="absolute top-full left-0 w-full mt-1 bg-gray-800 border border-gray-600 rounded-lg shadow-2xl z-50 max-h-60 overflow-y-auto">
                            {timesLiga.map(item => <button key={item.time_id} onClick={() => { setTimeVisitante(String(item.time_id)); setMenuVisitanteAberto(false); }} className="w-full flex items-center gap-3 p-3 hover:bg-gray-700 border-b border-gray-700/50 text-left"><img src={item.times.escudo} className="w-6 h-6" /><span className="text-sm font-bold">{item.times.nome}</span></button>)}
                        </div>
                        )}
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16">
                    {jogosDaRodada.length === 0 && <p className="text-gray-600 text-center py-10 col-span-2 border border-gray-800 border-dashed rounded">Nenhum confronto nesta fase.</p>}
                    {jogosDaRodada.map((jogo) => <MatchCard key={jogo.id} jogo={jogo} />)}
                </div>
            </>
            )}
          </div>

          {/* CLASSIFICAÇÃO (Apenas se NÃO for Mata-Mata) */}
          {liga?.tipo !== 'mata_mata' && (
            <>
