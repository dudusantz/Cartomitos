...
export async function salvarTime(prevState: { success: boolean, msg: string }, formData: FormData) {
    ...
    if (!termo) {
        return { success: false, msg: 'O campo de busca não pode estar vazio.' };
    }

    // 1. Buscar o time na API do Cartola
    const resultados = await buscarTimeCartola(termo);

    if (resultados.length === 0) {
        return { success: false, msg: `Nenhum time encontrado para o termo: "${termo}"` };
    }

    // Pega o primeiro resultado como o time a ser salvo
    const timeToSave = resultados[0];
    
    // 2. Checar se o time já está salvo no Supabase
    const { data: existe } = await supabase
        .from('times')
        .select('id')
        .eq('time_id_cartola', timeToSave.time_id)
        .single();

    if (existe) {
        return { success: false, msg: `O time "${timeToSave.nome}" já está cadastrado!` };
    }
    ...
    if (!error) {
        // Revalida as rotas para que o novo time apareça na lista imediatamente
...
// --- ATUALIZAÇÃO: SALVAR PLACAR MANUALMENTE (COM DESEMPATE) ---
export async function atualizarPlacarManual(
  ...
  // Se vier undefined, não atualiza o campo. Se vier null ou número, atualiza.
...
// --------------------------------------------------------------------------------------
// FUNÇÃO CRÍTICA: VERIFICA CLASSIFICADOS E GERA PRÓXIMA FASE
// --------------------------------------------------------------------------------------
async function verificarEAvancarFase(campeonatoId: number, rodadaAtual: number) {
  ...
  for (const jogo of jogosIdaList) {
    ...
    if (pA > pB) {
        ...
        const desempateA_Val = jogoDecisivo.desempate_casa;
        const desempateB_Val = jogoDecisivo.desempate_visitante;

        // Se NÃO TIVER PÊNALTI PREENCHIDO, BLOQUEIA O AVANÇO
        if (desempateA_Val === null || desempateA_Val === undefined || desempateB_Val === null || desempateB_Val === undefined) {
             console.log(`Empate no jogo ${jogo.id} sem pênaltis. Bloqueando avanço.`);
             return; // <--- TRAVA TOTAL. Se cair aqui, nada é salvo no banco.
        }

        let golsPenalA = 0;
        let golsPenalB = 0;

        // No jogo de IDA: Time A = Casa, Time B = Visitante.
        if (volta && volta.time_casa === jogo.time_visitante) {
            // Se inverteu mando na volta, o desempate_casa da volta é do Time B
            golsPenalB = desempateA_Val; 
            golsPenalA = desempateB_Val; 
        } else {
            golsPenalA = desempateA_Val;
            golsPenalB = desempateB_Val;
        }

        if (golsPenalA > golsPenalB) {
             vencedorId = jogo.time_casa;
             perdedorId = jogo.time_visitante;
        } else if (golsPenalB > golsPenalA) {
             vencedorId = jogo.time_visitante;
             perdedorId = jogo.time_casa;
        } else {
             // Empate nos pênaltis? Bloqueia também.
             return;
        }
    }
  ...
  // 4. Se chegou aqui, todos os confrontos foram resolvidos.
...
// --- FUNÇÃO PÚBLICA PARA O PAINEL CHAMAR MANUALMENTE ---
