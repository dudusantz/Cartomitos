        ...
        <div className="p-8 max-w-[1600px] mx-auto">
            ...
            {!liga?.ativo && podium.length > 0 && (
                <div className="bg-gradient-to-br from-[#1a1a1a] to-black p-8 rounded-3xl border border-yellow-600/30 relative overflow-hidden mb-10 shadow-2xl">
                    <div className="absolute top-0 right-0 p-10 opacity-10"><Trophy size={180} /></div>
                        ...
                        <div className="flex-1">
                            ...
                            {liga.data_fim && (
                                <p className="text-gray-500 text-sm flex items-center gap-2">
                                    <Calendar size={14} /> 
                                    Finalizado em: {new Date(liga.data_fim).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}
                                </p>
                            )}
                        </div>
                                    ...
                                    <div className="space-y-1">
                                        {pote1.map((t, idx) => (
                                            <div key={t.time_id} className={`flex justify-between items-center p-2 rounded ${idx < 2 ? 'bg-yellow-500/10 border border-yellow-500/30' : 'bg-white/5'}`}>
                                                ...
                                                <div className="flex gap-3 text-[10px] font-mono text-gray-500"><span>Gr.{t.gp_origem}</span><span className="text-white font-bold">{t.pts}pts</span></div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
